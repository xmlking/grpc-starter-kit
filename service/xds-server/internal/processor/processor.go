...
import (
    ...
    "github.com/xmlking/grpc-starter-kit/service/xds-server/internal/resources"
    "github.com/xmlking/grpc-starter-kit/service/xds-server/internal/watcher"
    "github.com/xmlking/grpc-starter-kit/service/xds-server/internal/xdscache"
)
...
func (p *Processor) ProcessFile(file watcher.NotifyMessage) {
	...
	if err != nil {
		log.Error().Err(err).Msg("error parsing yaml file")
		return
	}
	...
	if err := snapshot.Consistent(); err != nil {
        log.Error().Err(err).Msgf("snapshot inconsistency: %+v\n\n\n%+v", snapshot, err)
		return
	}
    log.Debug().Msgf("will serve snapshot %+v", snapshot)

	// Add the snapshot to the cache
	if err := p.cache.SetSnapshot(p.nodeID, snapshot); err != nil {
        log.Error().Err(err).Msgf("snapshot error for %+v",  snapshot)
		os.Exit(1)
	}
}
